# Ralph Coding Guidelines

This document defines coding standards and best practices for all code generated or modified by the Ralph automation system. Follow these guidelines strictly to ensure consistent, maintainable, and high-quality code.
...
if ! some_command "$arg"; then
    ralph_error "Failed to run some_command with arg: $arg"
    return 1
fi
...
result=$(some_command)
if [[ -z "$result" ]]; then
    ralph_warn "Command returned empty result"
fi
...
- Use existing utility functions (from ralph-lib.sh)
