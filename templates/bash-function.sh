...
backup_file() {
    ...
    # Validate input
    if [[ -z "$file_path" ]]; then
        ralph_error "backup_file: file_path is required"
        return 1
    fi

    if [[ ! -f "$file_path" ]]; then
        ralph_error "backup_file: File not found: ${file_path}"
        return 1
    fi

    # Create backup directory
    if [[ ! -d "$backup_dir" ]]; then
        ralph_debug "Creating backup directory: ${backup_dir}"
        mkdir -p "$backup_dir" || {
            ralph_error "backup_file: Failed to create backup directory"
            return 1
        }
    fi
    ...
    ralph_info "Backing up ${file_path} to ${backup_path}"

    if cp "$file_path" "$backup_path"; then
        ralph_info "Backup successful: ${backup_path}"
        echo "$backup_path"
        return 0
    else
        ralph_error "Backup failed for ${file_path}"
        return 1
    fi
}
